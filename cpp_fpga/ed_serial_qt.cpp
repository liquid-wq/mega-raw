...
std::pair<std::unique_ptr<EdSerial>, QString> find_everdrive(const QString& preferred) {
    // Reihenfolge: zuerst der vorgemerkte Port, dann echte USB-Geraete
    // (mit Hersteller-Kennung), zuletzt virtuelle Ports. Bluetooth-Ports
    // lassen sich oeffnen, antworten aber nie - jeder Fehlversuch kostet
    // Leerlaufschleife plus Handshake, das summiert sich auf Sekunden.
    QStringList candidates, likely, unlikely;
    if (!preferred.isEmpty()) candidates << preferred;
    for (const auto& info : QSerialPortInfo::availablePorts()) {
        const QString name = info.portName();
        if (candidates.contains(name)) continue;
        const bool bt = info.description().contains("Bluetooth", Qt::CaseInsensitive);
        if (!bt && info.hasVendorIdentifier()) likely << name;
        else unlikely << name;
    }
    candidates << likely << unlikely;
