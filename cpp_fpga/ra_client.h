...
public:
    ...
    // hardcore=true beruecksichtigt nur die Hardcore-Unlocks. Softcore zaehlt
    // dagegen beides, weil ein Hardcore-Unlock den Softcore-Unlock einschliesst.
    std::set<long long> ra_unlocks(long long gameid, const std::string& user,
                                   const std::string& token, bool hardcore = false);
