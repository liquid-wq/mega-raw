...
std::set<long long> RaClient::ra_unlocks(long long gameid, const std::string& user, const std::string& token, bool hardcore) {
    std::set<long long> ids;
    try {
        Json d = request_({{"r", "startsession"}, {"g", std::to_string(gameid)},
                            {"u", user}, {"t", token}});
        for (const auto& kv : d.entries()) {
            const std::string key = to_lower_str(kv.first);
            if (key.find("unlock") == std::string::npos) continue;
            if (hardcore && key.find("hardcore") == std::string::npos) continue;
